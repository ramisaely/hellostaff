...
<body class="full_vh">
    ...
    <!-- <a class="logo" href="{{ url('/') }}"><img src="{!! url('img/logo-helloStaff.svg') !!}"></img></a> -->
	    <section class="login_title">
	        ...
	        <p>{!! trans('auth.new_user') !!}&nbsp;&nbsp;&nbsp; <a href="{{ route('register_agency') }}" class="green">{!! trans('auth.sign_up') !!}</a></p>
	    </section>
	    ...
	    <div class="content_login_user">
			...
			<section class="form_login_user">
				<form action="{{ route('post_login_agency') }}" method="POST" class="form_login">
					@csrf
					<div class="form_item_login">
						<label class="green" for="">Email</label>
						<input type="text" class="input_login_user required" name="email" data-msg="{!! trans('staff_register.invalid_field') !!}" placeholder="{!! trans('auth.your_mail') !!}" autocomplete="off"/>
					</div>
					<div class="form_item_login">
